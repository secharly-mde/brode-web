...
export default function NutritionGrid() {
  ...
  return (
    <div 
      ref={gridRef}
      className="bg-white rounded-3xl p-8 md:p-10 shadow-xl shadow-saddle-brown/5 border border-saddle-brown/10"
    >
      <div className="grid grid-cols-2 md:grid-cols-4 gap-y-10 gap-x-6">
        {stats.map((stat, index) => (
          <div 
            key={index}
            className={`flex flex-col items-center text-center ${index > 0 ? 'md:border-l border-saddle-brown/10' : ''} transition-all duration-1000 transform ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
            style={{ transitionDelay: isVisible ? stat.delay : '0ms' }}
          >
            <span className="text-4xl md:text-5xl font-serif text-saddle-brown mb-2">
